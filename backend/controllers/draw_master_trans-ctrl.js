...
getDrawMasterTransList = async (req, res) => {
    await Draw_master_trans.find({ drawMasterId : req.params.id}, (err, dwt_data) => {
        if (err) {
        ...
        for(let data of dwt_data) {
            ...
            dummy_arr['dwt_customer_name']    = data['customer_details'];
...
createDrawMasterTrans = async (req,res) => {
    ...
    req_obj['shortCode']  = body.dwt_short_code;    
...
updateDrawMasterTrans = async (req,res) => {
    ...
    Draw_master_trans.findOne({_id : req.params.id},(err,di_data) => {
        if(err){
            return res.status(400).json({err,message : 'customer not found..!'});
        }
        var paid_amount = di_data.paidAmount;
        di_data['paidAmount']  = paid_amount + body.dwt_paid_amount;
        di_data['paidDate']    = new Date();
        console.log(di_data);
        // di_data.save(function(err,result){
        //     const statuscode = (err)?400:200;   
...
getDrawMasterTransDataById = async (req, res) => {
    ...
    //         const dummy_arr = {};
    //         dummy_arr['dwt_id']            = data['_id'];        
    //         dummy_arr['dwt_name']            = data['name'];        
    //         dummy_arr['dwt_date']            = data['drawDate'];   
    //         dummy_arr['dwt_installments']    = data['installments'];
    //         dummy_arr['dwt_amount']          = data['drawAmount'];      
    //         dummy_arr['dwt_short_code']       = data['shortCode'];  
    //         ret_arr.push(dummy_arr);
    //     }
    //     return res.status(200).json({ success: true, data: ret_arr })
    // });
